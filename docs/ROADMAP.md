...
KVLab position-native campaign directories are now self-contained at merged KVLab revision `ffc406abedcaf2b69cd89d804b52c3ebf7cd93ee`: the canonical campaign specification is persisted atomically with the manifest and observed records. ProspectEngine pins this campaign contract revision and exposes verified per-policy logical byte accounting plus observed metric baseline/candidate/delta values without deriving an automatic scientific ranking. Representative-model and benchmark gates remain open until qualifying executions exist.
