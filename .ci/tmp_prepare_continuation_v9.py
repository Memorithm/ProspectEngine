...
# v3 includes the original integration, terminal ownership fix, test cleanup,
# corrected module insertion, import fixes, and explicit JSON error conversion.
...
def one(old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(f'v9 continuation anchor drift count={count}: {old!r}')
    source = source.replace(old, new, 1)

# Clippy fixes which are not part of v3.
...
# Execution/inspection hardening.
...
def test_one(old: str, new: str) -> None:
    ...
    count = source.count(old)
    if count != 1:
        raise SystemExit(f'v9 test anchor drift count={count}: {old[:100]!r}')
    source = source.replace(old, new, 1)
