...
impl Temp {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("prospect-record-tests-{}-{}", std::process::id(), ID.fetch_add(1, Ordering::Relaxed)));
        fs::create_dir(&path).unwrap(); Self(path)
    }
    fn join(&self, name: &str) -> PathBuf { self.0.join(name) }
}
impl Drop for Temp { fn drop(&mut self) { let _ = fs::remove_dir_all(&self.0); } }
fn canonical(value: &Value) -> String { serde_json::to_string(value).unwrap() }
fn fixture() -> (ExecutionRecord, String) {
    ...
    (ExecutionRecord::verify_against_bundle(&payload, &bundle).unwrap(), bundle)
}
fn verify_command(record: &Path, bundle: &Path) -> std::process::Output {
    Command::new(env!("CARGO_BIN_EXE_prospect")).arg("verify-execution-record").arg(record).arg(bundle).output().unwrap()
}

#[test]
fn published_record_roundtrips_without_byte_normalization_or_implicit_resume() {
    let temp = Temp::new(); let (record, bundle) = fixture();
    let target = temp.join("record.json"); let input = temp.join("bundle.json");
    fs::write(&input, &bundle).unwrap();
    publish_execution_record(&record, &target).unwrap();
    assert_eq!(fs::read_to_string(&target).unwrap(), record.canonical_json());
...
fn existing_destination_is_never_overwritten_and_temporary_is_removed() {
    let temp = Temp::new(); let (record, _) = fixture(); let target = temp.join("record.json");
    fs::write(&target, "keep existing contents").unwrap();
    assert!(publish_execution_record(&record, &target).is_err());
    assert_eq!(fs::read_to_string(&target).unwrap(), "keep existing contents");
    assert_eq!(fs::read_dir(&temp.0).unwrap().count(), 1);
}

#[test]
fn directory_destination_is_preserved() {
    let temp = Temp::new(); let (record, _) = fixture(); let target = temp.join("directory");
    fs::create_dir(&target).unwrap();
    assert!(publish_execution_record(&record, &target).is_err());
    assert!(target.is_dir()); assert_eq!(fs::read_dir(&temp.0).unwrap().count(), 1);
}
...
fn dangling_destination_symlink_is_not_followed_or_removed() {
    ...
    let temp = Temp::new(); let (record, _) = fixture(); let target = temp.join("record.json");
    let missing = temp.join("absent"); symlink(&missing, &target).unwrap();
    assert!(publish_execution_record(&record, &target).is_err());
    assert_eq!(fs::read_link(&target).unwrap(), missing);
    assert!(!missing.exists()); assert_eq!(fs::read_dir(&temp.0).unwrap().count(), 1);
}
...
fn wrong_input_and_malformed_records_return_no_success_json() {
    ...
    let target = temp.join("record.json"); let input = temp.join("bundle.json");
    publish_execution_record(&record, &target).unwrap();
    let mut changed: Value = serde_json::from_str(&bundle).unwrap(); changed["state"] = json!(99);
    fs::write(&input, canonical(&changed)).unwrap();
    let mismatch = verify_command(&target, &input);
    assert_eq!(mismatch.status.code(), Some(1)); assert!(mismatch.stdout.is_empty());
    fs::write(&input, bundle).unwrap(); fs::write(&target, "{broken").unwrap();
    let malformed = verify_command(&target, &input);
    assert_eq!(malformed.status.code(), Some(1)); assert!(malformed.stdout.is_empty());
}

#[test]
fn command_requires_exactly_two_paths() {
    for args in [vec![], vec!["record"], vec!["record", "bundle", "extra"]] {
        let output = Command::new(env!("CARGO_BIN_EXE_prospect")).arg("verify-execution-record").args(args).output().unwrap();
...
fn both_inputs_share_the_existing_static_symlink_rejection() {
    ...
    let temp = Temp::new(); let (record, bundle) = fixture();
    let target = temp.join("record.json"); let input = temp.join("bundle.json");
    fs::write(&target, record.canonical_json()).unwrap(); fs::write(&input, bundle).unwrap();
    let record_link = temp.join("record.link"); let bundle_link = temp.join("bundle.link");
    symlink(&target, &record_link).unwrap(); symlink(&input, &bundle_link).unwrap();
...
fn oversized_files_are_rejected_by_the_shared_reader() {
    ...
    for large in [&target, &input] {
        fs::OpenOptions::new().write(true).open(large).unwrap().set_len(16 * 1024 * 1024 + 1).unwrap();
        let output = verify_command(&target, &input);
        assert_eq!(output.status.code(), Some(1)); assert!(output.stdout.is_empty());
        fs::write(&target, record.canonical_json()).unwrap(); fs::write(&input, &bundle).unwrap();
    }
