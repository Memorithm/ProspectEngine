...
impl Temp {
    fn new() -> Self {
        let path = std::env::temp_dir().join(format!("prospect-live-journal-{}-{}", std::process::id(), NEXT.fetch_add(1, Ordering::SeqCst)));
...
fn file_sink_preserves_exact_entries_and_acknowledges_after_write() {
    let temp = Temp::new(); let path = temp.path().join("run.jsonl");
    ...
    drop(sink); assert!(path.is_file());
}

#[test]
fn existing_files_and_directories_are_never_overwritten() {
    let temp = Temp::new(); let path = temp.path().join("existing");
...
fn dangling_and_live_symlink_destinations_are_not_followed() {
    ...
    let temp = Temp::new(); let target = temp.path().join("target");
    let link = temp.path().join("link"); symlink(&target, &link).unwrap();
    assert!(FileJournal::create(&link).is_err()); assert!(!target.exists());
...
fn unix_journal_is_owner_only() {
    ...
    let temp = Temp::new(); let path = temp.path().join("private");
...
fn invalid_entry_poisons_sink_and_preserves_acknowledged_prefix() {
    let temp = Temp::new(); let path = temp.path().join("run");
    let mut sink = FileJournal::create(&path).unwrap();
...
fn multiple_lines_and_oversized_entry_fail_before_writing() {
    for bytes in [b"{}\n{}\n".to_vec(), {
        let mut value = vec![b'x'; MAX_JOURNAL_ENTRY_BYTES]; value.push(b'\n'); value
    }] {
        let temp = Temp::new(); let path = temp.path().join("run");
        let mut sink = FileJournal::create(&path).unwrap();
        assert!(sink.append_record(&bytes).is_err());
        assert!(sink.is_poisoned()); assert_eq!(fs::metadata(path).unwrap().len(), 0);
    }
}

#[test]
fn cli_usage_errors_have_no_success_json() {
    for args in [vec!["inspect-execution-journal"], vec!["inspect-execution-journal", "a"],
                 vec!["inspect-execution-journal", "a", "b", "c"]] {
        let output = Command::new(env!("CARGO_BIN_EXE_prospect")).args(args).output().unwrap();
...
fn cli_rejects_missing_invalid_utf8_and_oversized_input_without_json() {
    let temp = Temp::new(); let journal = temp.path().join("run"); let bundle = temp.path().join("bundle");
    fs::write(&bundle, "{}").unwrap();
    for variant in 0..3 {
        match variant {
            0 => {},
            1 => fs::write(&journal, [0xff]).unwrap(),
            _ => { fs::File::create(&journal).unwrap().set_len(16 * 1024 * 1024 + 1).unwrap(); },
        }
        let output = Command::new(env!("CARGO_BIN_EXE_prospect"))
            .arg("inspect-execution-journal").arg(&journal).arg(&bundle).output().unwrap();
