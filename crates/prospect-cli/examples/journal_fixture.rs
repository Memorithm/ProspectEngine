...
use prospect_dispatch::execution::record::journal::{
    EngineIdentity, JournalCapture, evaluate_registered_bundle_journaled,
};
...
impl ProspectiveEngine<i32, i32> for Fixture {
    ...
    fn evaluate(&self, state: &i32, intervention: &i32) -> Result<i32, Self::Error> {
        if *intervention == 2 {
            ...
            if self.mode == "--fail-second" {
                return Err("fixture failure, not a model failure");
            }
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bundle = ScenarioBundle::new(
        "example.journal_run",
        AdapterBinding::from_metadata(&fixture.adapter_metadata()?),
        Some(7),
        10,
        (1..=3)
    ...
    input_file.write_all(bundle.canonical_json()?.as_bytes())?;
    input_file.sync_all()?;
    drop(input_file);
    ...
    if let Some(error) = run.journal_error() {
        return Err(io::Error::other(error.to_string()).into());
    }
    drop(journal);
    println!(
        "{}",
        serde_json::to_string(&inspect_execution_journal_files(journal_path, bundle_path)?)?
    );
    Ok(())
}
