...
impl fmt::Display for KvlabKvRealModelEvidenceError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NonCanonicalJson => formatter.write_str("KVLab real-model evidence JSON is not canonical"),
            Self::UnsupportedSchema => formatter.write_str("unsupported KVLab real-model evidence schema"),
            Self::EmptyText(field) => write!(formatter, "KVLab real-model evidence field {field} must not be empty"),
            Self::InvalidRunRevision => formatter.write_str("KVLab run repository revision must be a lowercase full Git SHA"),
            Self::InvalidSha256(field) => write!(formatter, "KVLab real-model evidence field {field} must be a lowercase SHA-256 digest"),
            Self::EmbeddedEviction(error) => write!(formatter, "invalid embedded KV eviction handoff: {error}"),
            Self::BaselineLogicalBytesMismatch => formatter.write_str("baseline logical KV bytes do not match the embedded eviction"),
            Self::CandidateLogicalBytesMismatch => formatter.write_str("candidate logical KV bytes do not match the embedded eviction"),
            Self::EmptyMetrics => formatter.write_str("KVLab real-model evidence must contain at least one metric"),
            Self::DuplicateMetric(name) => write!(formatter, "duplicate KVLab observed metric {name}"),
            Self::UnknownMetricKind(kind) => write!(formatter, "unknown KVLab observed metric kind {kind}"),
            Self::UnknownMetricPreference(preference) => write!(formatter, "unknown KVLab observed metric preference {preference}"),
            Self::NonFiniteMetric(name) => write!(formatter, "KVLab observed metric {name} contains a non-finite value"),
            Self::MetricDeltaMismatch(name) => write!(formatter, "KVLab observed metric {name} delta does not match candidate minus baseline"),
...
impl fmt::Display for ObservedKvEvidenceModelError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            ...
            Self::ContextMismatch => "observed KV eviction records do not share one experimental context",
            Self::BaselineMismatch => "observed KV eviction records do not share one paired full-cache baseline",
            Self::DuplicateOutcome => "observed KV eviction evidence contains duplicate logical outcomes",
            Self::MissingEvidence => "no observed real-model evidence matches this KV eviction outcome",
        })
    }
}
...
mod tests {
    ...
    use super::{
        KvlabKvRealModelEvictionEvidenceV1, MetricPreference, ObservedKvEvidenceModelError,
        ObservedKvEvictionEvidenceModel, ObservedMetricKind, KVLAB_KV_REAL_MODEL_EVIDENCE_REVISION,
    };
    ...
    fn parses_observed_record_and_preserves_metric_semantics() {
        let record = KvlabKvRealModelEvictionEvidenceV1::from_canonical_json(&record_json(3, '3', 0.78)).unwrap();
        assert_eq!(record.eviction().outcome().logical_evicted_bytes(), 128);
        assert_eq!(record.metrics()[0].kind(), ObservedMetricKind::Quality);
        assert_eq!(record.metrics()[0].preference(), MetricPreference::HigherIsBetter);
    ...
    fn rejects_tampered_delta_and_logical_bytes() {
        let mut value: serde_json::Value = serde_json::from_str(&record_json(3, '3', 0.78)).unwrap();
    ...
    fn observed_model_uses_paired_baseline_and_exact_candidate_records() {
        let first = KvlabKvRealModelEvictionEvidenceV1::from_canonical_json(&record_json(3, '3', 0.78)).unwrap();
        let second = KvlabKvRealModelEvictionEvidenceV1::from_canonical_json(&record_json(2, '4', 0.74)).unwrap();
    ...
    fn model_rejects_context_or_baseline_drift() {
        ...
        let mut context_value: serde_json::Value = serde_json::from_str(&record_json(2, '4', 0.74)).unwrap();
        ...
        let mut baseline_value: serde_json::Value = serde_json::from_str(&record_json(2, '4', 0.74)).unwrap();
