...
use prospect_dispatch::execution::{ExecutableAdapterRegistry, record::{PayloadCodecs, evaluate_registered_bundle_bound}};
...
impl VersionedAdapter for Add {
    fn adapter_metadata(&self) -> Result<AdapterMetadata, AdapterMetadataError> {
        ...
        AdapterMetadata::new("example.add", version, None,
            vec![AdapterCapability::new("example.evaluate", version)?])
    }
}
fn main() {
    let bundle = ScenarioBundle::new("example.record_run", AdapterBinding::from_metadata(&Add.adapter_metadata().unwrap()),
        Some(7), 10, vec![
            BundleScenario::new(ScenarioId::new("a").unwrap(), 1),
            BundleScenario::new(ScenarioId::new("b").unwrap(), 2),
        ], None, None).unwrap();
    let mut adapters = ExecutableAdapterRegistry::new(); adapters.register(Add).unwrap();
    let bound = evaluate_registered_bundle_bound(&bundle, &adapters,
        &MetricRegistry::<i32, i32>::new(), &DecisionPolicyRegistry::<i32, i32>::new(),
        &EvaluationControl::new(1), |_| {}).unwrap();
    let record = bound.capture_record(&RunId::new("example-run-001").unwrap(),
        PayloadCodecs::new("example.i32.v1", "example.error_text.v1").unwrap(),
        |signature| Ok(signature.to_string()), |error| Ok(error.to_string())).unwrap();
