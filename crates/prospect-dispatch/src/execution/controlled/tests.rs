...
fn complete_evaluation_retains_identity_without_invoking_scoring_or_policy() {
    ...
    assert_eq!(
        report.adapter().adapter_id().as_str(),
        "prospect.controlled_fixture"
    );
...
fn quota_returns_explicit_partial_bundle_without_a_winner() {
    ...
    let (metrics, policies) = registries();
    let report = evaluate_registered_bundle_controlled(
        &input(REVISION, "metric.fixture", "policy.fixture"),
        &adapters,
        &metrics,
        &policies,
    ...
    assert!(matches!(
        ...
        BatchStatus::Interrupted(InterruptionReason::EvaluationLimitReached)
    ));
...
fn all_requirements_are_resolved_even_for_controlled_evaluation() {
    ...
    let (metrics, policies) = registries();
    for bundle in [
        input("other-revision", "metric.fixture", "policy.fixture"),
        input(REVISION, "metric.missing", "policy.fixture"),
        input(REVISION, "metric.fixture", "policy.missing"),
    ] {
        let result = evaluate_registered_bundle_controlled(
            &bundle,
            &adapters,
            &metrics,
            &policies,
            &EvaluationControl::new(2),
            |_| panic!("no progress before successful preflight"),
        );
...
fn expired_deadline_blocks_all_registered_engine_calls() {
    ...
    assert!(matches!(
        report.evaluation().status(),
        BatchStatus::Interrupted(InterruptionReason::DeadlineReached)
    ));
    assert_eq!(calls.load(Ordering::SeqCst), 0);
}
