...
fn input() -> String {
    ScenarioBundle::new(
        "fixture.recovery",
        AdapterBinding::from_metadata(&adapter()),
        Some(7),
        10,
        (1..=3)
        ...
        emitter
            .append(Event::Initialized {
                header: Header {
                    ...
                    bundle_sha256: digest(input().as_bytes()),
                    adapter: serde_json::to_value(adapter()).unwrap(),
                    implementation: implementation(),
                    codecs: codecs(),
                    max_evaluations: quota,
...
fn successes(n: usize) -> Vec<Event> {
    (0..=n)
        .flat_map(|i| {
            [
                Event::CallStarted { target: target(i) },
                Event::CallSucceeded {
                    target: target(i),
                    payload: (10 + i).to_string(),
                },
            ]
        })
        .collect()
...
fn expectations(journal: &str, semantics: RestartSemantics) -> RestartExpectations {
    RestartExpectations::new(
        RestartAnchors::new(
            &RunId::new("external-run").unwrap(),
...
fn check(journal: &str) -> RestartPreflight {
    preflight_journal_restart(
        journal,
        &input(),
        &expectations(journal, RestartSemantics::PureIndependent),
        &"b".repeat(64),
    )
    .unwrap()
}
...
fn known_failure_requires_reconciliation_with_or_without_terminal() {
    for terminal in [false, true] {
        ...
        events.push(Event::CallFailed {
            target: target(2),
            error_payload: "failed".into(),
        });
        if terminal {
            events.push(Event::Finished {
        ...
        assert!(
            result
                .blockers
                .contains(&RestartBlocker::FailedCallRequiresReconciliation)
        );
        assert!(!result.continuation_preparation_allowed);
    }
}
...
fn complete_and_fully_returned_unclosed_runs_are_not_scheduled_again() {
    for terminal in [false, true] {
        ...
        assert!(
            result
                .blockers
                .contains(&RestartBlocker::NoNeverStartedCandidates)
        );
        assert_eq!(
            result.blockers.contains(&RestartBlocker::AlreadyCompleted),
            terminal
        );
        assert!(!result.continuation_preparation_allowed);
    }
...
fn external_journal_anchor_rejects_suffix_deletion_and_coherent_rehashing() {
    ...
    if let Event::CallSucceeded { payload, .. } = changed.last_mut().unwrap() {
        *payload = "99".into();
    }
...
fn independently_retained_run_implementation_codec_and_adapter_are_exact() {
    ...
    let mutations: Vec<Mutation> = vec![
        ...
        Box::new(|v| {
            v["codecs"] =
                serde_json::to_value(PayloadCodecs::new("other.i32", "other.error").unwrap())
                    .unwrap();
        }),
    ];
...
fn strict_expectation_parser_rejects_noncanonical_unknown_missing_and_bad_fields() {
    ...
    for payload in [
        ...
        "null".into(),
        "{".into(),
        " ".repeat(MAX_RESTART_EXPECTATION_BYTES + 1),
    ] {
