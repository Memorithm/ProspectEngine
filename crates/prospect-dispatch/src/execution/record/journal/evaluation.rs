...
use super::{
    CallTarget, Emitter, EngineIdentity, Event, Header, JournalError, JournalSink, SCHEMA,
    Terminal, validate_header,
};
...
impl<I, S, E> JournalRun<I, S, E> {
    ...
    pub fn state(&self) -> JournalRunState {
        if self.journal_error.is_some() {
            return JournalRunState::JournalFailed;
        }
    ...
    pub fn engine_error(&self) -> Option<&E> {
        match self.evaluation.status() {
            BatchStatus::EngineFailed {
                error: CallFailure::Engine(error),
                ..
            } => Some(error),
            _ => None,
        }
    ...
    pub fn failed_call(&self) -> Option<CallTarget> {
        match self.evaluation.status() {
            BatchStatus::EngineFailed {
                scenario,
                error: CallFailure::Engine(_),
            } => Some(match scenario {
                Some(s) => CallTarget::Scenario {
    ...
    pub fn never_started(&self) -> Vec<&Scenario<I>> {
        ...
        if let BatchStatus::EngineFailed {
            scenario: Some(s),
            error: CallFailure::BlockedBeforeEngine,
    ...
    pub fn interruption(&self) -> Option<InterruptionReason> {
        ...
        match self.evaluation.status() {
            BatchStatus::Interrupted(reason) => Some(*reason),
            _ => None,
        }
    }
    ...
    pub fn into_completed_batch(self) -> Result<BatchResult<I, S>, Box<Self>> {
        if self.state() != JournalRunState::Completed {
        ...
        match evaluation.into_completed_batch() {
            Ok(batch) => Ok(batch),
            Err(evaluation) => Err(Box::new(Self {
                input_json,
                adapter,
                evaluation,
                journal_error,
...
fn invoke<W: JournalSink + ?Sized, FS, FE, S, E>(
    session: &RefCell<Session<'_, W, FS, FE>>,
    target: CallTarget,
...
where
    ...
    FE: FnMut(&E) -> Result<String, String>,
{
    if !session.borrow_mut().emit(Event::CallStarted {
        target: target.clone(),
    }) {
...
where
    ...
    W: JournalSink + ?Sized,
    FS: FnMut(&E::Signature) -> Result<String, String>,
    FE: FnMut(&E::Error) -> Result<String, String>,
{
    ...
    fn baseline(&self, state: &State) -> Result<Self::Signature, Self::Error> {
        invoke(&self.session, CallTarget::Baseline, || {
            self.engine.baseline(state)
        })
    }
    fn evaluate(&self, state: &State, intervention: &I) -> Result<Self::Signature, Self::Error> {
        let target = {
            ...
            let Some(id) = session.ids.get(session.next).cloned() else {
                session.error =
                    Some(ExecutionRecordError::Invalid("journal input order exhausted").into());
                return Err(CallFailure::BlockedBeforeEngine);
            };
            session.next += 1;
            CallTarget::Scenario { id }
        };
        invoke(&self.session, target, || {
            self.engine.evaluate(state, intervention)
        })
    }
}
...
pub fn evaluate_registered_bundle_journaled<State, I, S, E, M, P, W, FS, FE>(
    ...
    capture: JournalCapture<'_, W, FS, FE>,
    ...
    FE: FnMut(&E) -> Result<String, String>,
{
    ...
    let engine = adapters.engine(id).ok_or_else(|| {
        BoundEvaluationError::Dispatch(BundleExecutionError::RegistryInvariant(id.to_owned()))
    })?;
    let header = Header {
        schema: SCHEMA.into(),
        ...
        adapter: serde_json::to_value(&adapter)
            .map_err(|e| BoundEvaluationError::Input(e.into()))?,
    ...
    let scenarios = bundle
        .scenarios()
        .iter()
        .map(|s| Scenario::new(s.id().clone(), s.intervention().clone()))
        .collect();
    ...
    let wrapped = RecordingEngine {
        engine,
        session: RefCell::new(session),
    };
    let evaluation =
        evaluate_batch_controlled(&wrapped, bundle.state(), scenarios, control, |_| {});
    ...
    if session.error.is_none() {
        let terminal = match evaluation.status() {
            ...
            BatchStatus::Interrupted(reason) => Some(Terminal::Interrupted {
                reason: match reason {
        ...
        if let Some(terminal) = terminal {
            session.emit(Event::Finished { terminal });
        } else {
            session.error =
                Some(ExecutionRecordError::Invalid("unexpected journal evaluator state").into());
        }
    }
    Ok(JournalRun {
        input_json,
