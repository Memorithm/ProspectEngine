...
fn bundle(state: i32) -> ScenarioBundle<i32, i32> {
    ScenarioBundle::new("experiment.record_fixture",
        AdapterBinding::new("prospect.record_fixture", version(),
            Some(UpstreamBinding::new("memorithm.fixture", REV).unwrap())).unwrap(),
        Some(7), state,
        vec![BundleScenario::new(ScenarioId::new("a").unwrap(), 1),
             BundleScenario::new(ScenarioId::new("b").unwrap(), 2)], None, None).unwrap()
}
fn codecs() -> PayloadCodecs { PayloadCodecs::new("fixture.i32.v1", "fixture.text.v1").unwrap() }
fn evaluate(control: &EvaluationControl, fail: u8) -> BoundBundleEvaluation<i32, i32, &'static str> {
    let mut adapters = ExecutableAdapterRegistry::new();
    adapters.register(Engine { calls: Arc::new(AtomicUsize::new(0)), fail }).unwrap();
    evaluate_registered_bundle_bound(&bundle(10), &adapters, &MetricRegistry::<i32, i32>::new(),
        &DecisionPolicyRegistry::<i32, i32>::new(), control, |_| {}).unwrap()
}
fn capture(bound: &BoundBundleEvaluation<i32, i32, &'static str>) -> ExecutionRecord {
    bound.capture_record(&RunId::new("run-001").unwrap(), codecs(), |s| Ok(s.to_string()), |e| Ok(e.to_string())).unwrap()
}
fn record() -> ExecutionRecord { capture(&evaluate(&EvaluationControl::new(2), 0)) }
fn mutate(edit: impl FnOnce(&mut Value)) -> Result<ExecutionRecord, ExecutionRecordError> {
    ...
    edit(&mut value);
    ExecutionRecord::verify_against_bundle(&canonical(&value).unwrap(), &bundle(10).canonical_json().unwrap())
}
...
fn roundtrip_binds_exact_bundle_and_preserves_payload_bytes() {
    ...
    let record = bound.capture_record(&RunId::new("unicode-run-é").unwrap(), codecs(),
        |s| Ok(format!("value={s}\n\0é")), |e| Ok(e.to_string())).unwrap();
    let decoded = ExecutionRecord::verify_against_bundle(record.canonical_json(), bound.input_json()).unwrap();
...
fn quota_interruption_retains_prefix_and_exact_never_started_suffix() {
    ...
    assert!(matches!(record.terminal(), RecordTerminal::Interrupted { reason: RecordInterruption::EvaluationLimitReached }));
}
...
fn failed_baseline_and_failed_candidate_are_not_retry_queues() {
    ...
    assert!(matches!(baseline.terminal(), RecordTerminal::Failed { scenario_id: None, error_payload } if error_payload == "baseline\nerror"));
    ...
    assert!(matches!(candidate.terminal(), RecordTerminal::Failed { scenario_id: Some(id), error_payload } if id == "b" && error_payload == "candidate\nerror"));
}

#[test]
fn encoder_failure_leaves_original_report_available() {
    let bound = evaluate(&EvaluationControl::new(2), 2);
    let result = bound.capture_record(&RunId::new("run").unwrap(), codecs(),
        |_| Err("codec refused value".to_owned()), |e| Ok(e.to_string()));
...
fn cancellation_after_last_return_is_not_promoted_during_capture() {
    ...
    let bound = evaluate_registered_bundle_bound(&bundle(10), &adapters, &MetricRegistry::<i32, i32>::new(),
        &DecisionPolicyRegistry::<i32, i32>::new(), &control, |update| {
            if matches!(update.event, ProgressEvent::ScenarioCompleted { index: 1, .. }) { token.cancel(); }
        }).unwrap();
    let record = capture(&bound);
    assert_eq!(record.summary().state, "interrupted");
...
fn exact_input_snapshot_precedes_baseline_call() {
    let calls = Arc::new(AtomicUsize::new(0));
    let mut adapters = ExecutableAdapterRegistry::new();
    adapters.register(Engine { calls: Arc::clone(&calls), fail: 0 }).unwrap();
    struct SerializationSpy(Arc<AtomicUsize>);
    impl Serialize for SerializationSpy {
        fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
            assert_eq!(self.0.load(Ordering::SeqCst), 0, "input serialized after engine start");
            serializer.serialize_i32(10)
        }
    }
    struct SpyEngine(Engine);
    impl ProspectiveEngine<SerializationSpy, i32> for SpyEngine {
        type Signature = i32; type Error = &'static str;
        fn baseline(&self, _: &SerializationSpy) -> Result<i32, Self::Error> { self.0.baseline(&10) }
        fn evaluate(&self, _: &SerializationSpy, i: &i32) -> Result<i32, Self::Error> { self.0.evaluate(&10, i) }
    }
    impl VersionedAdapter for SpyEngine {
        fn adapter_metadata(&self) -> Result<AdapterMetadata, prospect_adapter::AdapterMetadataError> { self.0.adapter_metadata() }
    }
    let old = bundle(10);
    let spy_bundle = ScenarioBundle::new("experiment.record_fixture", old.adapter().clone(), Some(7),
        SerializationSpy(Arc::clone(&calls)), old.scenarios().to_vec(), None, None).unwrap();
    let mut spy_adapters = ExecutableAdapterRegistry::new();
    spy_adapters.register(SpyEngine(Engine { calls: Arc::clone(&calls), fail: 0 })).unwrap();
    let result = evaluate_registered_bundle_bound(&spy_bundle, &spy_adapters, &MetricRegistry::<i32, i32>::new(),
        &DecisionPolicyRegistry::<i32, i32>::new(), &EvaluationControl::new(2), |_| {}).unwrap();
...
fn rejects_altered_top_level_identity_and_admission_fields() {
    for (key, value) in [
        ("schema", json!("other")), ("evidence_kind", json!("observed")),
        ("run_id", json!(" ")), ("bundle_sha256", json!("0".repeat(64))),
        ("bundle_id", json!("experiment.other")), ("seed", json!(8)),
        ("max_evaluations", json!(1)), ("max_evaluations", json!(2.0)),
    ] {
...
fn rejects_unknown_missing_duplicate_or_noncanonical_fields() {
    ...
    assert!(mutate(|v| { v.as_object_mut().unwrap().remove("seed"); }).is_err());
    let record = record();
    let original = record.canonical_json();
    let duplicate = original.replacen("{", "{\"run_id\":\"other\",", 1);
    for payload in [duplicate, format!("{original}\n"), original[..original.len()-1].to_owned(), "[]".to_owned()] {
        assert!(ExecutionRecord::verify_against_bundle(&payload, &bundle(10).canonical_json().unwrap()).is_err());
    }
}
...
fn rejects_unknown_or_noncanonical_nested_adapter_metadata() {
    for edit in [
        ...
        ("upstream", json!({"component":"memorithm.fixture","revision":"different"})),
        ("capabilities", json!([])),
    ] {
...
fn rejects_prefix_suffix_gaps_duplicates_and_partial_completion() {
    ...
    assert!(mutate(|v| v["outcomes"].as_array_mut().unwrap().pop().map(|_| ()).unwrap()).is_err());
    ...
    assert!(ExecutionRecord::verify_against_bundle(&canonical(&value).unwrap(), &bundle(10).canonical_json().unwrap()).is_err());
}
...
fn rejects_impossible_terminal_details() {
    ...
    assert!(mutate(|v| v["terminal"] = json!({"state":"failed","scenario_id":"b","error_payload":"x"})).is_err());
    assert!(mutate(|v| v["terminal"] = json!({"state":"interrupted","reason":"evaluation_limit_reached"})).is_err());
    assert!(mutate(|v| v["terminal"] = json!({"state":"interrupted","reason":"deadline_reached"})).is_err());
    assert!(mutate(|v| v["terminal"] = json!({"state":"completed","error_payload":"unexpected"})).is_err());
}
...
fn rejects_invalid_codecs_and_oversized_encoded_values() {
    ...
    let bound = evaluate(&EvaluationControl::new(2), 0);
    let result = bound.capture_record(&RunId::new("run").unwrap(), codecs(),
        |_| Ok("x".repeat(MAX_ENCODED_PAYLOAD_BYTES + 1)), |e| Ok(e.to_string()));
...
fn envelope_and_input_byte_limits_fail_closed() {
    ...
    assert!(ExecutionRecord::verify_against_bundle(&oversized, &bundle(10).canonical_json().unwrap()).is_err());
...
fn canonical_format_rejects_nonfinite_json_metadata_without_decoding_payloads() {
    for invalid in ["NaN", "Infinity", "1e999"] {
        let payload = record().canonical_json().replace("\"max_evaluations\":2", &format!("\"max_evaluations\":{invalid}"));
        assert!(ExecutionRecord::verify_against_bundle(&payload, &bundle(10).canonical_json().unwrap()).is_err());
    }
    // Payload text is intentionally opaque, not automatically interpreted as a number.
    let bound = evaluate(&EvaluationControl::new(2), 0);
    let record = bound.capture_record(&RunId::new("run").unwrap(), codecs(), |_| Ok("NaN".to_owned()), |e| Ok(e.to_string())).unwrap();
