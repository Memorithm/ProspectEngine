...
fn completion_matches_existing_batch_without_changing_input_order() {
    ...
    let report = evaluate_batch_controlled(
        &engine,
        &(),
        candidates(),
...
fn empty_batch_still_requires_a_successful_baseline() {
    let engine = Fixture::default();
    let report =
        evaluate_batch_controlled(&engine, &(), Vec::new(), &EvaluationControl::new(0), |_| {});
...
fn cancellation_after_last_success_is_not_mislabeled_complete() {
    ...
    let report = evaluate_batch_controlled(&engine, &(), candidates(), &control, |update| {
        if matches!(
            update.event,
            ProgressEvent::ScenarioCompleted { index: 2, .. }
        ) {
            token.cancel();
        }
    });
...
fn terminal_notification_does_not_rewrite_finalized_state() {
    ...
    let token = control.cancellation_token();
    let report = evaluate_batch_controlled(&engine, &(), candidates(), &control, |update| {
        if matches!(update.event, ProgressEvent::Finished(_)) {
            token.cancel();
        }
    });
    assert!(control.cancellation_token().is_cancelled());
...
fn baseline_overrun_is_interrupted_not_successful_even_for_empty_batch() {
    ...
    let engine = Fixture {
        baseline: Box::new(move || {
            update_clock.set(deadline);
            Ok(10)
        }),
        ..Fixture::default()
    };
    let report = evaluate_with_clock(
        &engine,
        &(),
        Vec::new(),
        &EvaluationControl::new(0).with_deadline(deadline),
        |_| {},
        || clock.get(),
    );
    interrupted(&report, InterruptionReason::DeadlineReached);
...
fn last_candidate_overrun_is_preserved_but_not_promoted_to_complete() {
    ...
    let report = evaluate_with_clock(
        ...
        &(),
        candidates(),
        &EvaluationControl::new(3).with_deadline(deadline),
        |_| {},
        || clock.get(),
    );
    interrupted(&report, InterruptionReason::DeadlineReached);
...
fn callback_time_is_observed_at_next_checkpoint() {
    ...
    let report = evaluate_with_clock(
        ...
        &EvaluationControl::new(3).with_deadline(deadline),
        |update| {
            if matches!(update.event, ProgressEvent::BaselineCompleted) {
                clock.set(deadline);
            }
        },
        || clock.get(),
...
fn baseline_failure_preserves_error_and_never_started_input() {
    let engine = Fixture {
        baseline: Box::new(|| Err("baseline failed")),
        ..Fixture::default()
    };
    let report = evaluate_batch_controlled(
        &engine,
    ...
    assert!(
        matches!(report.status(), BatchStatus::EngineFailed { scenario: None, error } if *error == "baseline failed")
    );
    assert!(report.baseline().is_none());
    assert_eq!(report.pending().len(), 3);
    assert_eq!(*engine.calls.borrow(), vec![None]);
...
fn engine_error_takes_precedence_over_a_concurrent_cancellation() {
    ...
    let token = control.cancellation_token();
    let engine = Fixture {
        candidate: Box::new(move |_| {
            token.cancel();
            Err("actual engine failure")
        }),
        ..Fixture::default()
    };
    let report = evaluate_batch_controlled(&engine, &(), candidates(), &control, |_| {});
    assert_eq!(report.state(), ExecutionState::Failed);
    assert!(
        matches!(report.status(), BatchStatus::EngineFailed { error, .. } if *error == "actual engine failure")
    );
}
...
fn duplicate_ids_reject_before_any_engine_call_and_preserve_all_inputs() {
    ...
    let report =
        evaluate_batch_controlled(&engine, &(), input, &EvaluationControl::new(10), |update| {
            if let ProgressEvent::Finished(state) = update.event {
                events.push(state);
            }
        });
    assert_eq!(report.state(), ExecutionState::Rejected);
    assert!(
        matches!(report.status(), BatchStatus::DuplicateScenarioId(id) if id.as_str() == "candidate-1")
    );
...
fn progress_has_ordered_success_events_and_exactly_one_terminal_event() {
    ...
    for index in 0..3 {
        ...
        assert!(
            events[index + 1]
                .0
                .contains(&format!("candidate-{}", index + 1))
        );
...
fn failure_emits_no_success_event_for_the_failed_candidate() {
    let engine = Fixture {
        candidate: Box::new(|_| Err("failed")),
        ..Fixture::default()
    };
    let mut events = Vec::new();
    let report = evaluate_batch_controlled(
...
fn control_precedence_is_cancellation_then_deadline_then_candidate_quota() {
    ...
    assert_eq!(
        control.interruption_at(deadline, 0, 1),
