...
fn main() {
    let complete = evaluate_batch_controlled(
        &Add, &10, scenarios(), &EvaluationControl::new(3), |_| {},
    );
    let batch = complete.into_completed_batch().expect("all fixture calls succeed");
    println!("complete: baseline={}, candidates={}", batch.baseline(), batch.outcomes().len());
    ...
    let partial = evaluate_batch_controlled(&Add, &10, scenarios(), &control, |update| {
        println!("progress: {:?} ({}/{})", update.event, update.completed_scenarios, update.total_scenarios);
        if matches!(update.event, ProgressEvent::ScenarioCompleted { index: 0, .. }) {
            cancel.cancel();
        }
    });
    assert_eq!(partial.state(), ExecutionState::Interrupted);
    println!("interrupted: completed={}, never_started={}", partial.outcomes().len(), partial.pending().len());
